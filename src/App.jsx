  ...
  for (let i = 0; i < jsonData.length; i++) {
    ...
    if (currentSection && firstCell) {
      ...
      if (currentSection === 'yarn' || currentSection === 'fabric') {
        // For yarn/fabric: col B = consumption (g), col C = price (USD/KG), col D = cost
        consumption = parseFloat(row[1]) || 0;
        price = parseFloat(row[2]) || 0;
        // Convert grams to kilograms (divide by 1000) for cost calculation
        ...
        // Add if description exists (even if cost is 0)
        shouldAdd = true;
      } else if (currentSection === 'packaging') {
        // For packaging: col A = type, col B = notes, col C = cost
        const type = String(row[0] || '').trim();
        const notes = String(row[1] || '').trim();
        let cost = 0;

        // Try to get cost from the correct column
        if (row[3] !== undefined && row[3] !== '') {
          cost = parseFloat(row[3]) || 0;
        } else if (row[2] !== undefined && row[2] !== '') {
          cost = parseFloat(row[2]) || 0;
        }

        // Handle packaging data
        if (type === 'Standard Packaging' || type === 'Special Packaging') {
          // Check if this type already exists
          const existingIndex = parsedData.packaging.findIndex(item => item.type === type);
          if (existingIndex >= 0) {
            // Update existing entry
            parsedData.packaging[existingIndex] = {
              type,
              notes,
              cost
            };
 ...
 * @param {number} consumption - Consumption amount in grams
 * @param {number} price - Price per kilogram
 * @returns {number} - Calculated cost
 */
export const calculateMaterialCost = (consumption, price) => {
  // Convert consumption from grams to kilograms for calculation
  const consumptionKg = (parseFloat(consumption) || 0) / 1000;
  const pricePerKg = parseFloat(price) || 0;
  const result = consumptionKg * pricePerKg;
